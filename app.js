(() => {
  ...
  const CONFIG = {
    ...
    APP_TITLE: "IFCEngine",
    APP_BUILD: "20260521-asbuilt-no-shared-placement-transform",
  ...
  const state = {
    ...
    zeroPointSelectedFileIds: new Set(),
    explorerApi: null,
  ...
  function setBusy(busy) {
    ...
    if (ui.convertManualBtn) {
      ui.convertManualBtn.style.display = "none";
    ...
    if (ui.deliveryRunBtn) ui.deliveryRunBtn.disabled = busy || state.zeroPointSelectedFileIds.size === 0;
  }
  ...
  function getCachedAccessToken() {
    try {
      const token = window.sessionStorage?.getItem("ifcengine.accessToken");
  ...
  function setAccessToken(token) {
    ...
    try { window.sessionStorage?.setItem("ifcengine.accessToken", token); } catch {}
  ...
  function clearAccessToken() {
    ...
    try { window.sessionStorage?.removeItem("ifcengine.accessToken"); } catch {}
  }
  ...
  function buildUi() {
    ...
    titleCard.appendChild(el("div", "subtitle", "IFC-verktøy for prosjektdata, As built og nullpunkt."));

    const tabsCard = el("div", "card tabs-card");
    const projectTabBtn = el("button", "primary", "Prosjektdata");
    const fieldTabBtn = el("button", null, "As built");
    const deliveryTabBtn = el("button", null, "Zero Point");
    ...
    const filesHeader = el("div", "card-header", [
      el("div", null, [
        el("div", "label", "IFC-prosjektdata"),
        el("div", "subtitle", "IFC-filer i Trimble Connect-prosjektet.")
      ])
    ]);
    ...
    const localUploadBtn = el("button", null, "Konverter lokal JXL");
    ...
    localFileInput.accept = ".jxl,application/xml,text/xml";
    ...
    const jxlHeader = el("div", "card-header", [
      el("div", null, [
        el("div", "label", "As built"),
        el("div", "subtitle", "Konverter JXL-eksporter fra Field Data og målebok til IFC.")
      ])
    ]);
    ...
    const deliveryHeader = el("div", "card-header", [
      el("div", null, [
        el("div", "label", "Zero Point"),
        el("div", "subtitle", "Generer og last opp IFC world-filer med valgt nullpunkt.")
      ])
    ]);
    deliveryCard.appendChild(deliveryHeader);
    const zeroPointGrid = el("div", "coordinate-grid");
    const zeroPointInputs = {
      localEast: buildNumberField("Lokalt øst", "0.000"),
      localNorth: buildNumberField("Lokalt nord", "0.000"),
      absoluteEast: buildNumberField("Absolutt øst", "103650.000"),
      absoluteNorth: buildNumberField("Absolutt nord", "1263020.000")
    };
    Object.values(zeroPointInputs).forEach((field) => zeroPointGrid.appendChild(field.wrap));
    deliveryCard.appendChild(zeroPointGrid);
    const zeroPointList = el("div", "file-list");
    deliveryCard.appendChild(zeroPointList);
    const deliveryBtnRow = el("div", "btn-row");
    const deliveryRunBtn = el("button", "primary", "Generer IFCW");
    deliveryRunBtn.disabled = true;
    deliveryBtnRow.appendChild(deliveryRunBtn);
    deliveryCard.appendChild(deliveryBtnRow);
    const zeroPointResult = el("div", "status", "Velg IFC-modeller og generer world-filer.");
    deliveryCard.appendChild(zeroPointResult);
  ...
  function buildNumberField(label, value) {
    const input = document.createElement("input");
    input.type = "number";
    input.step = "0.001";
    input.value = value;
    const wrap = el("label", "coordinate-field", [
      el("span", null, label),
      input
    ]);
    return { wrap, input };
  ...
  function getZeroPointFiles() {
    return (Array.isArray(state.fileList) ? state.fileList : [])
      .filter((file) => /\.ifc$/i.test(String(file?.name || "")));
  }

  function getIfcWorldFilename(filename) {
    return String(filename || "modell.ifc").replace(/\.ifc$/i, ".ifcw");
  }

  function zeroPointValues() {
    const read = (field) => Number.parseFloat(ui.zeroPointInputs?.[field]?.input?.value || "0");
    return {
      localEast: read("localEast"),
      localNorth: read("localNorth"),
      absoluteEast: read("absoluteEast"),
      absoluteNorth: read("absoluteNorth")
    };
  }

  function formatCoordinate(value) {
    const num = Number(value);
    if (!Number.isFinite(num)) throw new Error("Nullpunktverdier må være gyldige tall.");
    return num.toFixed(3);
  }

  function buildIfcWorldContent(values) {
    return [
  ...
  function renderZeroPointList() {
    if (!ui.zeroPointList) return;
    ui.zeroPointList.innerHTML = "";
    const files = getZeroPointFiles();
    state.zeroPointSelectedFileIds.forEach((id) => {
      if (!files.some((file) => file.id === id)) state.zeroPointSelectedFileIds.delete(id);
    });

    if (!files.length) {
      ui.zeroPointList.appendChild(el("div", "empty-state", "Trykk \"Oppdater prosjektdata\" for å hente IFC-modeller."));
      setBusy(state.busy);
      return;
    }

    for (const file of files) {
      const checked = state.zeroPointSelectedFileIds.has(file.id);
      const row = el("label", `file-item pending${checked ? " selected" : ""}`);
      const checkbox = document.createElement("input");
      checkbox.type = "checkbox";
      checkbox.value = file.id;
      checkbox.checked = checked;
      checkbox.addEventListener("change", () => {
        if (checkbox.checked) state.zeroPointSelectedFileIds.add(file.id);
        else state.zeroPointSelectedFileIds.delete(file.id);
        renderZeroPointList();
      });
      row.appendChild(checkbox);
      row.appendChild(el("div", "file-info", [
        el("div", "file-name", file.name || "(uten navn)"),
        el("div", "file-meta", file.path || "IFC-modell")
      ]));
      row.appendChild(el("span", "file-status pending", getIfcWorldFilename(file.name)));
      ui.zeroPointList.appendChild(row);
    }
    setBusy(state.busy);
  }
  ...
  function renderFileList() {
    ...
    ui.fileCount.textContent = allConvertibleFiles.length
      ? `${allConvertibleFiles.length} IFC-fil${allConvertibleFiles.length === 1 ? "" : "er"}`
      : "";
    ...
    if (!currentFiles.length && !childFolders.length) {
      const empty = el("div", "empty-state", "Trykk \"Oppdater prosjektdata\" for å hente IFC-filer fra prosjektet.");
    ...
    for (const file of currentFiles) {
      ...
      selector.name = state.manualSelectionMode ? "manualIfcFile" : "ifcFile";
      ...
      info.appendChild(el("div", "file-meta", isConvertible ? (file.path || "IFC-fil") : (file.path || "Ikke IFC")));
  ...
  function isSourceProjectFile(file) {
    return /\.ifc$/i.test(String(file?.name || ""));
  }
  ...
  function showHint(message, show = true) {
    ...
    ui.hint.innerHTML = `<span class="hint-icon">Info</span>${message}`;
  }
  ...
  function getExpectedOutputExtensions(file) {
    ...
    if (name.endsWith(".ifc")) return [".ifcw"];
  ...
  function getFileConversionState(file) {
    if (/\.ifc$/i.test(String(file?.name || ""))) {
      return { className: "pending", label: "IFC" };
    }
  ...
  async function generateZeroPointWorldFiles() {
    try {
      await ensureReady();
      const selectedFiles = getZeroPointFiles().filter((file) => state.zeroPointSelectedFileIds.has(file.id));
      if (!selectedFiles.length) {
        if (ui.zeroPointResult) {
          ui.zeroPointResult.textContent = "Ingen IFC-modeller valgt.";
          ui.zeroPointResult.className = "status error";
        }
        setBusy(false);
        return;
      }

      const content = buildIfcWorldContent(zeroPointValues());
      setBusy(true);
      if (ui.zeroPointResult) {
        ui.zeroPointResult.textContent = "Laster opp IFCW-filer til Trimble Connect...";
        ui.zeroPointResult.className = "status working";
      }

      const results = [];
      for (const file of selectedFiles) {
        const fileName = getIfcWorldFilename(file.name);
        const proxyRes = await callProxy("uploadWorldFile", {
          token: state.accessToken,
          projectId: state.project.id,
          projectLocation: state.project.location,
        ...
        const ok = proxyRes.ok && proxyRes.json?.ok;
        results.push({
          ok,
          source: file.name,
          fileName,
          result: proxyRes.json || proxyRes.text
        });
        if (ok) markFileConverted(file, fileName);
      }

      state.lastZeroPointResult = results;
      renderFileList();
      const okCount = results.filter((result) => result.ok).length;
      const failed = results.filter((result) => !result.ok);
      if (failed.length) {
        if (ui.zeroPointResult) {
          ui.zeroPointResult.textContent = `${okCount} IFCW lastet opp, ${failed.length} feilet. Se tekniske detaljer.`;
          ui.zeroPointResult.className = "status error";
        }
        setDebug({ action: "uploadWorldFile", content, results });
      } else {
        if (ui.zeroPointResult) {
          ui.zeroPointResult.textContent = `${okCount} IFCW-fil${okCount === 1 ? "" : "er"} lastet opp.`;
          ui.zeroPointResult.className = "status success";
        }
        setDebug({ action: "uploadWorldFile", content, results });
      }
    } catch (err) {
      console.error(err);
      if (ui.zeroPointResult) {
        ui.zeroPointResult.textContent = `Feil: ${err?.message || String(err)}`;
        ui.zeroPointResult.className = "status error";
      }
      setDebug({ action: "uploadWorldFile", error: err?.message || String(err), stack: err?.stack });
    } finally {
      setBusy(false);
    }
  }
  ...
  async function refreshKofList() {
    try {
      ...
      if (result.action === "listProjectExplorer") {
        ...
        state.fileList = (Array.isArray(result.files) ? result.files : []).filter(isSourceProjectFile);
      }
      ...
      if (state.fileList.length === 0) {
        setStatus("Ingen IFC-filer funnet", "neutral");
      } else {
        setStatus(`Fant ${state.fileList.length} IFC-fil${state.fileList.length === 1 ? "" : "er"}`, "success");
      }
      ...
      const debugPayload = {
        ...
        ifcWorldCount: state.fileList.filter((file) => isConvertedOutputCurrent(file)).length,
  ...
  function wireUi() {
    ...
    ui.deliveryRunBtn.addEventListener("click", generateZeroPointWorldFiles);
